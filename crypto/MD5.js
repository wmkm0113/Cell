...
const _rotateAndAdd = function (x = 0, y = 0, _count) {
    return CryptoUtils.SAFE_ADD(x.safeRotateLeft(_count), y);
}

const _md5FF = function (i = 0, j = 0, k = 0, x = 0, y = 0) {
    return CryptoUtils.SAFE_ADD(((i & j | ~i & k)), x + y);
}

const _md5GG = function (i = 0, j = 0, k = 0, x = 0, y = 0) {
    return CryptoUtils.SAFE_ADD(((i & k | j & ~k)), x + y);
}

const _md5HH = function (i = 0, j = 0, k = 0, x = 0, y = 0) {
    return CryptoUtils.SAFE_ADD((i ^ j ^ k), x + y);
}

const _md5II = function (i = 0, j = 0, k = 0, x = 0, y = 0) {
    return CryptoUtils.SAFE_ADD(((i ^ (j | ~k))), x + y);
}
...
export default class MD5 extends Crypto {
    ...
    _calculate(_tmp) {
        ...
        this._md5Cycle(_tmp, 0, 1, _md5FF, RC_FF, [7, 12, 17, 22]);
        this._md5Cycle(_tmp, 1, 5, _md5GG, RC_GG, [5, 9, 14, 20]);
        this._md5Cycle(_tmp, 5, 3, _md5HH, RC_HH, [4, 11, 16, 23]);
        this._md5Cycle(_tmp, 0, 7, _md5II, RC_II, [6, 10, 15, 21], true);
    ...
    _md5Cycle(tmp, begin, step, cycleFunc, rc, r, final = false) {
        ...
        for (let i = 0; i < 16; i++) {
            ...
            this._hash[_index] = _rotateAndAdd(
